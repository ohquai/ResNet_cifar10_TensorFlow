...
def create_variables(name, shape, initializer=tf.contrib.layers.xavier_initializer(), is_fc_layer=False):
    ...
    """
    if not is_fc_layer:
        regularizer = tf.contrib.layers.l2_regularizer(scale=FLAGS.weight_decay_conv)
        new_variables = tf.get_variable(name, shape=shape, initializer=initializer, regularizer=regularizer)
    else:
        regularizer = tf.contrib.layers.l2_regularizer(scale=FLAGS.weight_decay_fc)
        new_variables = tf.get_variable(name, shape=shape, initializer=initializer, regularizer=regularizer)
...
def conv_bn_relu_layer(input_layer, filter_shape, stride):
    ...
    kernel = create_variables(name='conv', shape=filter_shape)
...
def bn_relu_conv_layer(input_layer, filter_shape, stride):
    ...
    kernel = create_variables(name='conv', shape=filter_shape)
    conv_layer = tf.nn.conv2d(relu_layer, kernel, strides=[1, stride, stride, 1], padding='SAME')
    return conv_layer
...
def residual_block(input_layer, output_channel, first_block=False):
    ...
    with tf.variable_scope('conv1_in_block'):
        if first_block:
            kernel = create_variables(name='conv', shape=[3, 3, input_channel, output_channel])
            conv1 = tf.nn.conv2d(input_layer, filter=kernel, strides=[1, 1, 1, 1], padding='SAME')
        else:
    ...
    # When the channels of input layer and conv2 does not match,
    # we add zero pads to increase the depth of input layers
    ...
    for i in range(n):
        with tf.variable_scope('conv1_%d' % i, reuse=tf.AUTO_REUSE):
            if i == 0:
                ...
                conv1 = residual_block(layers[-1], output_channel=16)
...
def test_graph(train_log_dir='logs'):
    """
    ...
    sess = tf.Session()
    sess.run(init)
    summary_writer = tf.train.SummaryWriter(train_log_dir, sess.graph)

    return result, summary_writer


if __name__ == '__main__':
    result, summary_writer = test_graph()
